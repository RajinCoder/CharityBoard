...
type Need = 'CLOTHING' | 'VOLUNTEERS' | 'BLANKETS' | 'PILLOWS' | 'TOILETRIES' | 'FOOD' | 'WATER';
const needsOptions: Need[] = ["CLOTHING", "VOLUNTEERS", "BLANKETS", "PILLOWS", "TOILETRIES", "FOOD", "WATER"];
...
const AddListing = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const { data, error } = await supabase.from('ListingTable').insert([
      { name, contact, location, needs: selectedNeeds },
    ]);
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {needsOptions.map((need) => (
              <button
                ...
                className={`px-3 py-1 text-sm font-medium rounded-full transition-colors ${selectedNeeds.includes(need) ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-700'}`}
              >
                {need}
              </button>
            ))}
            ...
            {submitAttempted && selectedNeeds.length === 0 && (
              <p className="text-red-500 text-xs italic">Please select at least one need.</p>
            )} 
          </div>
